define([
  ...
  class DropDown extends Backbone.View() {
    ...
    getDefaults() {
      return {
        load: this.defaults.load,
        openList: this.defaults.openList,
        closeList: this.defaults.closeList,
        scrollToItem: this.defaults.scrollToItem
      };
    }
    ...
    select(value) {
      value = String(value);
      const option = _.find(this.options, (option) => option.getValue() === value);
      if (option) {
        option.select();
        return;
      }
